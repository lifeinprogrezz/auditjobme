// PublicAudit — the deliverable of the company-audit step (issue #82). Reached
// at /a/:username/:slug, published by AuditGenerator once a user generates an
// audit (audits.is_published, RLS already opens SELECT to anon for those rows).
// This is the artifact standard from BRAND.md's audit carve-out: white
// background, brand-aligned via the company's own accent color, noindex — a
// page built to be sent to the company itself, not part of the app's own
// ink-glass token layer. Read-only render of audit_data; no generation, no
// LLM calls, no auth required. Restores the page deleted in 9d48a10 (unrouted
// at the time) — same data shape AuditGenerator's own "results" stage renders,
...
/* Shape of the audit_data JSON blob (LLM-generated by AuditGenerator). Every field optional. */
interface StatItem { value?: string; label?: string; delta?: string; source_url?: string }
interface SourceRef { url?: string; name?: string }
...
interface ProposalItem {
  ...
  solution?: string;
  how_it_works?: string;
  target_effort_impact?: string;
}
interface AuditData {
  accent?: string;
  company?: { company?: string; company_desc?: string; competitors?: string[]; stats?: StatItem[]; role?: string; role_url?: string };
  diagnosis?: { headline?: string; sub?: string; findings?: FindingItem[] };
  proposals?: { headline?: string; sub?: string; proposals?: ProposalItem[] };
  about?: { headline?: string; headline_accent?: string; stats?: { value?: string; label?: string }[]; columns?: { skill?: string; proof?: string }[] };
  pains?: { key_quote?: string; quote_source?: string; quote_url?: string };
  cv?: { name?: string };
  roleCtx?: { audit_label?: string; role_type?: string };
}

/* ═══════════════════ REUSABLE RENDER HELPERS ═══════════════════ */
function safeAccent(hex?: string) {
  if (!hex || !/^#[0-9a-fA-F]{6}$/.test(hex)) return "#3f5a63";
  const r = parseInt(hex.slice(1, 3), 16), g = parseInt(hex.slice(3, 5), 16), b = parseInt(hex.slice(5, 7), 16);
  const brightness = (r * 299 + g * 587 + b * 114) / 1000;
  // On a white page an accent only needs to stay legible against white, i.e. not
  // too pale — a too-dark accent is still fine here (unlike the dark-theme tool).
  if (brightness > 225) {
    const factor = 0.35;
...
/** Keeps a public audit out of search results (BRAND.md artifact standard). Added
 *  on mount, removed on unmount so it never leaks onto any other route. */
function useNoindex() {
  useEffect(() => {
...
export default function PublicAudit() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Always resolve through the owner's profile, even when there is only one
      // candidate -- a slug is unique per owner, not globally, so a wrong
      // :username in the path must never render someone else's audit (#90).
      const ownerIds = [...new Set(matchingAudits.map((audit) => audit.user_id).filter(Boolean))];
      ...
      const matchedAudit = resolveAuditMatch(matchingAudits, profiles || [], username);
      ...
      setData(matchedAudit.audit_data as AuditData);
      setLoading(false);
    }
    load();
  }, [username, slug]);
  ...
  useEffect(() => {
    if (data?.company?.company) {
      document.title = `${data.company.company} audit`;
    }
  }, [data]);

  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#ffffff", color: "#6b7280", fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: ".7rem", letterSpacing: ".1em", textTransform: "uppercase" as const }}>
        Loading audit
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center", background: "#ffffff", color: "#101820", fontFamily: "'Plus Jakarta Sans', sans-serif", gap: 12 }}>
        <p style={{ fontSize: "1.2rem", fontFamily: "'DM Sans', sans-serif", fontWeight: 800 }}>404</p>
        <p style={{ fontSize: ".75rem", color: "#6b7280" }}>{error || "Audit not found"}</p>
      </div>
    );
  }
  ...
  const NAV_LINKS = ["research", "diagnosis", "proposals", "about"];
  ...
  return (
    <>
      <style>{`
        ...
        :root{--bg:#ffffff;--surface:#f6f7f8;--text:#101820;--muted:#6b7280;--border:rgba(16,24,32,.12);--accent:${accent}}
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav{position:fixed;top:0;left:0;right:0;height:48px;display:flex;align-items:center;justify-content:space-between;padding:0 clamp(1.2rem,4vw,3rem);z-index:100;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid var(--border);background:rgba(255,255,255,.92)}
        ...
        .hero{min-height:100vh;display:flex;flex-direction:column;justify-content:flex-end;padding:0 clamp(1.2rem,4vw,3rem) clamp(2rem,4vw,3rem);background:var(--bg);color:var(--text);position:relative}
        .hero-label{font-size:.62rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:1.2rem;display:flex;align-items:center;gap:.5rem}
        .hero-dot{width:8px;height:8px;background:var(--accent);display:inline-block}
        .hero h1{font-family:'DM Sans',sans-serif;font-weight:800;font-size:clamp(1.9rem,5.5vw,4.2rem);line-height:1.05;letter-spacing:-.03em;margin-bottom:.8rem;color:var(--text)}
        .hero h1 .accent{color:var(--accent)}
        .hero-sub{font-size:.83rem;color:var(--muted);max-width:540px;line-height:1.7;margin-bottom:2rem}
        .hero-bottom{display:flex;justify-content:space-between;align-items:flex-end}
        .hero-author{text-align:right}
        .hero-author strong{font-size:.83rem;display:block;color:var(--text)}
        .hero-author span{font-size:.7rem;color:var(--muted)}
        ...
        .about-section{position:relative;padding:clamp(3rem,8vw,5rem) clamp(1.2rem,4vw,3rem);text-align:center;background:var(--bg);color:var(--text)}
        .about-section::before{content:'';position:absolute;top:0;left:clamp(1.2rem,4vw,3rem);right:clamp(1.2rem,4vw,3rem);height:1px;background:var(--border)}
        .about-inner{max-width:700px;margin:0 auto}
        .about-stats{display:grid;grid-template-columns:repeat(3,1fr);max-width:700px;margin:2rem auto;border:1px solid var(--border);border-radius:8px;overflow:hidden}
        .about-stat{padding:1.5rem;border-right:1px solid var(--border);text-align:center}
        .about-stat:last-child{border-right:none}
        .about-stat-val{font-family:'DM Sans',sans-serif;font-weight:800;font-size:clamp(1.3rem,3vw,2rem);margin-bottom:.3rem;color:var(--text)}
        .about-stat-label{font-size:.56rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--muted)}
        .about-cols{display:grid;grid-template-columns:repeat(3,1fr);max-width:700px;margin:2rem auto;gap:2rem;text-align:center}
        .about-col h4{font-family:'DM Sans',sans-serif;font-weight:800;font-size:.8rem;letter-spacing:.06em;text-transform:uppercase;margin-bottom:.6rem;color:var(--text)}
        .about-col p{font-size:.75rem;line-height:1.7;color:var(--muted)}
        ...
        @media(max-width:600px){
          ...
          .about-stats{grid-template-columns:1fr}.about-stat{border-right:none;border-bottom:1px solid var(--border)}.about-stat:last-child{border-bottom:none}
        }
      `}</style>
      ...
      <section className="hero" id="hero">
        ...
        <Anim delay={0.7}>
          <div className="hero-bottom">
            <span style={{ fontSize: ".58rem", fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--muted)" }}>SCROLL DOWN</span>
      ...
      <section className="section" id="research">
        ...
        <Anim delay={0.1}>
          ...
          {(data.company?.competitors?.length ?? 0) > 0 && (
      ...
      <section className="about-section" id="about">
        <Anim><div className="sec-label">04 · ABOUT</div></Anim>
        <Anim delay={0.1}>
      ...
      <div className="footer">
        ...
        <div style={{ marginTop: "12px", fontSize: ".5rem", fontWeight: 400, letterSpacing: ".08em", opacity: 0.6 }}>
          MADE WITH{" "}
