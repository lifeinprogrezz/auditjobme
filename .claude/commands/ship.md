---
description: Deploy Northgoing to Vercel production, confirm the deploy is healthy, and report
allowed-tools: Bash
---
Ship the current state of the Northgoing web app to Vercel production. Run from the repo root; stop and report at the first hard failure. This is the deploy half of the deploy→check→iterate loop (`/verify` is the local half).
...
2. **Deploy to production** — `vercel deploy --prod --yes` (the repo is linked to the `auditjobme` Vercel project — that is the PROJECT name, which the Northgoing rebrand did NOT change; northgoing.com is a domain attached to it. The auth'd CLI handles the rest). Capture the deployment URL it prints.
